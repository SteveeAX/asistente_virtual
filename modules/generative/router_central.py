...
class RouterCentral:
    ...
    def _process_classic_route(self, user_text: str, route_decision: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # El intent_manager original no procesa, solo detecta
            # Necesitamos crear una respuesta básica aquí o delegar a improved_app.py
            if detected_intent:
                response = f"Comando reconocido: {detected_intent}"
                confidence = 0.95
            else:
                response = "Comando no reconocido"
                confidence = 0.1
            
            # Enriquecer el resultado con metadatos del router
            enriched_result = {
                'success': True,
                'route': 'classic',
                'response': response,
            ...
            logger.debug(f"Procesamiento clásico exitoso: {detected_intent}")
