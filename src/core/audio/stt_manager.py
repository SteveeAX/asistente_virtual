...
SAMPLE_RATE = int(os.getenv('MIC_SAMPLE_RATE', '48000'))  # Configurable por micrófono
CHUNK_SIZE = int(SAMPLE_RATE / 10)  # Ajusta automáticamente al sample rate
LANGUAGE_CODE = "es-US"  # Mejor modelo de entrenamiento que es-EC

# Inicializar cliente STT con manejo de errores
client_stt = None
try:
    from google.cloud import speech
    from google.protobuf import duration_pb2
    client_stt = speech.SpeechClient()
    logger.info("Cliente Google Cloud STT inicializado exitosamente")
except Exception as e:
    logger.warning(f"No se pudo inicializar Google Cloud STT: {e}")
    logger.info("STT funcionará en modo degradado (sin reconocimiento de voz en la nube)")
    client_stt = None
...
def get_stt_config(adaptation_phrases: list = None):
    ...
    """
    # Verificar si el módulo speech está disponible
    if client_stt is None or 'speech' not in globals():
        logger.warning("Módulo speech no disponible, retornando None")
        return None
...
def stream_audio_and_transcribe(adaptation_phrases: list = None) -> str | None:
    ...
    def stt_task():
        ...
        # Verificar si el cliente STT está disponible
        if client_stt is None:
            logger.warning("Cliente Google Cloud STT no disponible, STT deshabilitado")
            transcript_result[0] = ""  # Retornar string vacío
            return
...
def listen_for_confirmation(timeout=7) -> str:
    ...
    def stt_confirmation_task():
        ...
        # Verificar si el cliente STT está disponible
        if client_stt is None:
            logger.warning("Cliente Google Cloud STT no disponible, confirmación STT deshabilitada")
            transcript_result[0] = ""  # Retornar string vacío
            return
